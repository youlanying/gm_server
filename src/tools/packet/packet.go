...
func (p *Packet) ReadRawData() (ret []byte, err error) {
	if p.pos+4 > uint(len(p.data)) {
		err = errors.New("read rawdata header failed")
		return
	}

	size, _ := p.ReadU32()
	if p.pos+uint(size) > uint(len(p.data)) {
		err = errors.New("read rawdata data failed")
		return
	}
...
func (p *Packet) WriteRawData(v []byte) {
	p.WriteU32(uint32(len(v)))
	p.data = append(p.data, v...)
}
