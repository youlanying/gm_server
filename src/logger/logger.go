...
func CheckLogFile() {
	file, err := OpenLogFile(_logfile_base, os.O_APPEND|os.O_WRONLY|os.O_CREATE, 0666)
	if err == nil {
		log.New(io.MultiWriter(file, os.Stderr), "", log.LstdFlags)
...
func InitLogger(cfgBasePath string, logfile string, maxSize int64) {
	...
	if filepath.IsAbs(logfile) { // start with slash, just open
		...
		fullpath = path.Join(cfgBasePath, "", logfile)
	}
	...
	dir = fmt.Sprint(cfgBasePath + "/" + dir)
	if _, loc_stat := os.Stat(dir); loc_stat != nil {
...
func Getlogfilemaxsize() int64 {
	return _logfile_maxsize
}

func GetDefaultCallDepth() int {
	return default_calldepth
}
